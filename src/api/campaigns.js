...
// (임시) 호출자 정보: viewerId/viewerRole은 쿼리로 받고 company는 DB 조회로 확정
async function getViewer(req) {
  const viewerId = Number(req.query.viewerId || req.query.adminId);
  const viewerRole = (req.query.viewerRole || req.query.adminRole || '').trim();
...
const commonInclude = [
  { model: User, as: 'User', attributes: ['id', 'name', 'email', 'company'], required: false },   // 담당자
...
router.get('/', async (req, res) => {
  try {
    ...
    if (viewerRole === '대행사 어드민') {
      if (!viewerCompany) return res.status(400).json({ message: '대행사 정보가 없습니다.' });
      whereClause = {
        [Op.or]: [
...
// POST /api/campaigns — 생성(테넌트 검증) 후 include 붙여 반환
