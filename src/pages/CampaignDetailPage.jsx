                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-200">
                        ...
                        <div className="px-8 py-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
                            <h3 className="text-2xl font-bold text-gray-900 flex items-center space-x-3">
                                <span className="flex items-center justify-center w-10 h-10 bg-blue-100 rounded-xl">
                                    <FileText className="w-5 h-5 text-blue-600" />
                                </span>
                                <span>업무 내용 상세보기</span>
                            </h3>
                            {titleDetailModal.post && (
                                <p className="text-sm text-gray-600 mt-2 ml-10">
                                    캠페인: <span className="font-medium text-gray-900">{campaign?.name}</span>
                                </p>
                            )}
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto px-8 py-6">
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <textarea
                                        value={titleDetailModal.post?.title || ''}
                                        onChange={(e) => setTitleDetailModal(prev => ({
                                            ...prev,
                                            post: { ...prev.post, title: e.target.value }
                                        }))}
                                        className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 resize-none text-sm leading-relaxed"
                                        placeholder="업무 내용을 입력하세요..."
                                        rows="10"
                                    />
                                    <p className="text-xs text-gray-500 mt-2 ml-1">
                                        수정 후 "저장" 버튼을 클릭하면 변경사항이 적용됩니다
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="px-8 py-5 border-t border-gray-200 bg-gray-50 flex justify-end space-x-3">
                            <button
                                onClick={() => setTitleDetailModal({ isOpen: false, post: null })}
                                className="px-5 py-2.5 text-gray-700 bg-white border-2 border-gray-300 rounded-xl hover:bg-gray-50 hover:border-gray-400 font-medium transition-all duration-200 shadow-sm"
                            >
                                취소
                            </button>
                            <button
                                onClick={async () => {
                                    if (!titleDetailModal.post) return;

                                    try {
                                        // API 호출로 업무 내용 수정
                                        await api.put(`/api/campaigns/${campaignId}/posts/${titleDetailModal.post.id}`, {
                                            title: titleDetailModal.post.title
                                        });

                                        showSuccess('업무 내용이 수정되었습니다.');

                                        // 데이터 새로고침
                                        await fetchCampaignDetail();

                                        // 모달 닫기
                                        setTitleDetailModal({ isOpen: false, post: null });
                                    } catch (error) {
                                        console.error('업무 내용 수정 실패:', error);
                                        showError(`업무 내용 수정에 실패했습니다: ${error.response?.data?.detail || error.message}`);
                                    }
                                }}
                                className="px-6 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 font-medium transition-all duration-200 shadow-lg hover:shadow-xl flex items-center space-x-2"
                            >
